...
class Constants:  # to change variables quickly, easily and buglessly.
    ...
    # crash logging above 26 (might be fixed later)
    max_board_size: int = min(26, get_setting("max_board_size", 6))
    ...
    def from_json(json: dict):
        return CountUnion(
            ...
            (*(Element.from_str(element)
             for element in getordef(json, "elements", ())),),
            (*getordef(json, "meta", ()),)
        )
...
class FuncNumeric(Numeric):
    ...
    def __str__(self):
        match self.f.__name__:
            ...
            # don't use other lambdas
            case "<lambda>": return f"the square of {self.numeric}"
...
class SetCardProperty(AbstractEffect):
    ...
    def execute(self, **kwargs) -> bool:
        ...
        if L != 0:
            ...
            attr = attrs.pop(0)
            # shares: do not modify anything, and don't use this for what should be logged
...
class DamageRedirect(AbstractEffect):
    ...
    def execute(self, **kwargs) -> bool:
        ...
        while amount > 0 and len(targets) > 0 and len(from_) > 0:
            ...
            while len(sources) != 0:
                source = sources.pop()
                redirected = min(source.card.max_hp - source.hp,
...
class RandomTargets(AbstractEffect):
    ...
    def execute(self, **kwargs) -> bool:
        print(
            f"RandomTargets launched with target {kwargs['target_mode'].name} executed.")
        targets = AbstractEffect.targeted_objects(**kwargs)
    ...
    def execute(self, **kwargs) -> bool:
        ...
        for card in AbstractEffect.targeted_objects(**kwargs):
            ...
            kwargs["survey"].damage += card.damage(
                self.amount.eval(
                    **kwargs) * 10 // ifelse(kwargs["user"].state == State.cloudy, 12, 10),
                **kwargs)
        return False
...
class BoardResize(AbstractEffect):
    ...
    def execute(self, **kwargs):
        ...
        kwargs["board"].logs.append(
            f"boardsize|{player.namecode()}|{len(player.active)}")
...
class Passive:
    ...
    def execute(self, **kwargs):
        ...
        kwargs["source"] = self
        # card.name to handle whendefeated passive.
        kwargs["board"].logs.append(
            f"passive|{kwargs['user'].card.name}|{self.name}")
...
class ActiveCard:
    ...
    def change_state(self, new_state: State) -> bool:
        ...
        if self.state.value < new_state.value or new_state is State.default:
            ...
            if new_state is not State.discarded:
                self.board.logs.append(
                    f"-state|{self.namecode()}|{new_state.name}")
                return True
        return False
    ...
    def attack(self, attack: Attack, target, **kwargs) -> EffectSurvey:
        ...
        getorset(kwargs, "damage_mode", ifelse(self.state ==
                 State.monotonous, DamageMode.ignore_se, DamageMode.direct))
        ...
        if self.state == State.cloudy:  # overrides taunt
            if len(AbstractEffect.targeted_objects(**withfield(kwargs, "target_mode", TargetMode.foes))) == 0:
                ...
                kwargs["target_mode"] = TargetMode.target
                kwargs["main_target"] = rng.choice(AbstractEffect.targeted_objects(
                    **withfield(kwargs, "target_mode", TargetMode.foes)))
    ...
    def namecode(self):
        ...
        for i in range(len(self.owner.active)):
            if i > 25:
                ...
                self.board.logs.append(
                    "raw|Warning: board size is unsupported, causing logging issues.")
...
class Player:
    ...
    def add_charges(self, amount: int, source: ActiveCard | None = None):
        ...
        if amount != 0:
            self.commander.board.logs.append(
    ...
    def handdiscard(self, i: int) -> AbstractCard:
        ...
        self.commander.board.logs.append(
            f"discard|{self.namecode()}|{card.name}")
...
class Board:
    ...
    def __init__(self, player1: Player, player2: Player, autoplay: bool = True):
        ...
        self.logs.append(
            f"player|p1|{self.player1.name}|{self.player1.commander.card.name}|{self.player1.commander.card.max_hp}|{self.player1.commander.element}")
        self.logs.append(
            f"player|p2|{self.player2.name}|{self.player2.commander.card.name}|{self.player2.commander.card.max_hp}|{self.player2.commander.element}")
        ...
        self.logs.append(
            ifelse(self.active_player is self.player1, "-firstp|p1", "-firstp|p2"))
