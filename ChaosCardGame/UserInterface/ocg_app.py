...
from utility import toggle_mute, getsetting
...
class OcgGame:
    ...
    def start(self):
        ...
        self.running = True
        while self.running:
            from utility import getsetting, toggle_mute
            ...
            for event in pygame.event.get():
                if event.type == pygame.KEYUP and event.key == pygame.K_p:
                    print("----------- P KEY PRESSED ------------")
                    print(f"before it was {getsetting('mute', False)}")

                    toggle_mute()
                    from utility import getsetting
                    print(f"now its {getsetting('mute', False)}")
                    sound_handle( action_type="mute/unmute", channel=2,is_muted=getsetting('mute', False))
